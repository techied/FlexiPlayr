        ...
        FlexiUtils.api.hasVoted(event.getAuthor().getId()).whenComplete((hasVoted, e) -> {
            if (hasVoted) {
                ...
                Message message = event.getChannel().sendMessage(new EmbedBuilder()
                        ...
                        .setColor(Color.RED)
                        .setDescription("Since this command requires more processing power than usual, we kindly ask that you vote for free [here](https://discordbots.org/bot/339215794418352129) or you can donate [here](https://discord.gg/EkaAdNj) to permanently gain access to this command.\n\nOnce you've voted simply add the ✅ reaction to this message")
                ...
                FlexiUtils.waiter.waitForEvent(MessageReactionAddEvent.class, n -> n.getUser().getIdLong() == event.getAuthor().getIdLong(), evt -> {
                    FlexiUtils.api.hasVoted(event.getAuthor().getId()).whenComplete((voted2, e2) -> {
                        if (voted2) {
